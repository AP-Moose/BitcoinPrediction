...
if news_data.empty:
    ...
    try:
        ...
        logger.error(f"Error in model training or prediction: {str(e)}")
        st.error(f"An error occurred while processing the {model_type} model: {str(e)}")
        st.error("Please try again with a different model or contact support if the issue persists.")
